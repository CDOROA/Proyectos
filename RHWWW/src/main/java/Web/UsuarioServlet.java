...
public class UsuarioServlet extends HttpServlet {
	...
	{	
    	...
    	if(accion.equalsIgnoreCase("Cerrar")) {
			cerrarSesion(request, response, session);
		...
		{  
			if(session == null)
			{
				//System.out.print(session+" Entre");
	...
	private void validaCredencialesDeUsuario(HttpServletRequest request, HttpServletResponse response,HttpSession session) {
		try {
			...
			String usuario = String.valueOf(request.getParameter("txtUsuario"));
			String password = String.valueOf(request.getParameter("txtPassword"));
			...
			else
			{
				...
				RedireccionarVista(request, response,"../AdministadorVacantes?accion=Inicio");
			}
		} catch (Exception e) {
	...
	private void cerrarSesion(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		session.invalidate();
		RedireccionarVista(request, response, "Inicio.jsp");
	}
