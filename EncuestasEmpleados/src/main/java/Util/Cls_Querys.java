	...
	public static List<Querys> ConsultaTablaQuerysBD(Connection connBD)
	{
		...
		try
		{
			String qry = "SELECT 		DISTINCT proc_web AS proceso, "+
											...
											"descripcion, "+
											"estructura AS query "+
								"  FROM 	DCH.QUERYS where proc_web = '160' " +
...
// System.out.println(qry);
	...
	public static ResultSet EjecutarQuery(String qry1, String qry2, String qry3, String qry4, String qry5, String qry6,
			...
			Connection conexion, Empleado infoUsu) {
		...
		try {
			...
			InsertaLogEncuestas.insertarLog(new LogEncuestas(),cdo,cdo,"Error: Cls_Querys.EjecutarQuery Error: "+ex.getMessage().toString().replace("'", "")+" Usuario: "+infoUsu.getNoEmpleado(),infoUsu.getNoEmpleado());  
		}
		return rs;
