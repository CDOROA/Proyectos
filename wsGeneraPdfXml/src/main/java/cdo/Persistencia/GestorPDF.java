...
//                	}
	        ...
	        Cls_Logs.insertarLog(cdo,serie,folio,"Se ejecuta el query: "+(querys[0].replace("'", "`")));
		}catch(Exception e) {
			Cls_Logs.insertarLog(cdo,serie,folio,"No se pudo recuperar la cadena del PDF del folio: "+folio+", serie: "+serie);
			//System.out.println("Error al entrar al metodo RecuperaRuta: " + e);
		}
		return base64;
	}
	public static String recuperaXML(String cdo, Connection connBD, PreparedStatement pstmt,
		...
		try {			   
			...
			Cls_Logs.insertarLog(cdo,serie,folio,"No se pudo recuperar la cadena del XML del folio: "+folio+", serie: "+serie);
			//System.out.println("Error al entrar al metodo RecuperaRuta: " + e);
		}
		return xml;
	}
