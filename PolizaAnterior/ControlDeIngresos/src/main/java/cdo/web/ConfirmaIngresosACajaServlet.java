...
public class ConfirmaIngresosACajaServlet extends HttpServlet {
				...
				case "ConfirmaIngresosACaja.jsp":
					switch(operacion)
					{
						case "ConsultaInicialDeIngresos":
							...
							List<Ingresos> ListIngresosCapturados = consultaIngresosXParametro(request,session);
							...
							String listaJson = gson.toJson(listaInforIngresos);							
	...
	private String  validaPolizasDeDiasAnteriores(HttpServletRequest request, HttpSession session)
	{
		...
		if(infoUsu.getUname().equals(infoUsu.getUname_br()))
		 ...
		 {
			 polizasGeneradasCorrectamente=gestorIngresos.validaSiSeGeneroPolizaDiaAnterior(infoUsu, this.querys);
			 
		 }
		return polizasGeneradasCorrectamente;
	}
			
	private List<Ingresos> consultaIngresosXParametro(HttpServletRequest request, HttpSession session)
	{
		List<Ingresos> ListIngresosCapturados = new ArrayList<>();
		Usuario infoUsu= (Usuario) session.getAttribute("infoUsu");
		System.out.println( infoUsu );
		try
		{
			System.out.println( "...1");
			...
			ListIngresosCapturados = gestorIngresos.consultaIngresosXParametro(infoUsu, this.querys, ICapturado, tipoFecha);
	...
	private List<SemaforoIngresos> consultaDatosDelSemaforo(HttpServletRequest request, HttpSession session)
	{
		Usuario infoUsu= (Usuario) session.getAttribute("infoUsu");
		System.out.println("1.1");
		List<SemaforoIngresos> listaSemaforo = gestorIngresos.consultaDatosDelSemaforo(infoUsu, this.querys);
