...
import cdo.Persistencia.GestorCuentasContacto;
import cdo.Persistencia.GestorTipoContactoV2;

 
public class CuentasDepositoServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
 
    public CuentasDepositoServlet() {
        super();
    }
	...
	private String ConsultaTipoContacto(HttpServletRequest request, HttpSession session) {
	    ...
	    GestorCuentasContacto cuentasContacto = new GestorCuentasContacto();
		String cuentas="";
		try
		{
			cuentasContacto.init();
			cuentas = cuentasContacto.consultaCuentasDeposito(infoCliente.getCentro(), infoCliente.getCve_cliente());
			cuentasContacto.destroy();
		}
		catch(Exception ex)
		{
		...
		//System.out.println("Consulta cuentas deposito: " + cuentas);
		return cuentas;
	}
