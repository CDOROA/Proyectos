...
public class GestorMantenimientoCarritoCompras 
{
	...
	public ArticulosCarritoPedidoCompleto  consultaPedidoActualEnCarritoCompras(List<Querys> listaQuerys,InfoCliente infoCliente)
	{
		...
		catch(Exception ex)
		{
			...
			Log log=new Log(1,infoCliente.getCve_cdo(),infoCliente.getCve_cliente() ,0, "[Error: Consulta pedido actual en el carrito de compras,  Clase: GestorMantenimientoCarritoCompras.consultaPedidoActualEnCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
	...
	private ArticulosCarritoEncabezado llenaClaseEncabezadoArticulosCarrito(ResultSet rs)
	{
		...
		{
			System.out.println("[NuevoportalRamaCDO.- Error: Llenar clase encabezado del carrito,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
			Log log=new Log(1,0,0 ,0, "[Error: Llenar clase encabezado del carrito,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
			Cls_Log.insertaLog(log);
		}
		return header;		
	}
	...
	private List<ArticulosCarritoDetalle> llenaClaseDetalleArticulosCarrito(InfoCliente infoCliente, ResultSet rs)
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				//	System.out.println("....2");
					JSONObject JsonRs = consultaExistenciasEnWS(infoCliente, rs.getString("art_numart_cdo"),rs.getInt("ped_cant_p") );
					//System.out.println("....3");
					 
					detalle.setCantidadPedida(rs.getInt("ped_cant_p")); 

					/* SE recupera las cantidades de la base de datos.-*/
		...
		{
			System.out.println("[NuevoportalRamaCDO.- Error: Llenar clase detalle del carrito,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
			Log log=new Log(1,0,0 ,0, "[Error: Llenar clase detalle del carrito,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
			Cls_Log.insertaLog(log);
		}
	...
	private JSONObject consultaExistenciasEnWS(InfoCliente infoCliente, String num_art, int cantidad)
	{
		...
		{
			System.out.println("[NuevoportalRamaCDO.- Error: Invocar wsExistencias para consulta de exietncias x articulo ,  Clase: GestorMantenimientoCarritoCompras.consultaExistenciasEnWS,  Detalle: " + ex.getMessage().toString() +"]");
			Log log=new Log(1,infoCliente.getCve_cdo(),infoCliente.getCve_cliente() ,0, "[Error: Invocar wsExistencias para consulta de exietncias x articulo,  Clase: GestorMantenimientoCarritoCompras.consultaExistenciasEnWS,  Detalle: " + ex.getMessage().toString() +"]");
			Cls_Log.insertaLog(log);
		}		
	...
	public ArticulosCarritoPedidoCompleto  EiminaArticuloDelCarrito(List<Querys> listaQuerys,InfoCliente infoCliente,String articulo, int pedido)
	{
		...
		{
			System.out.println("[NuevoportalRamaCDO.- Error: Consulta pedido actual en el carrito de compras,  Clase: GestorMantenimientoCarritoCompras.EiminaArticuloDelCarrito,  Detalle: " + ex.getMessage().toString() +"]");
			Log log=new Log(1,infoCliente.getCve_cdo(),infoCliente.getCve_cliente() ,0, "[Error: Consulta pedido actual en el carrito de compras,  Clase: GestorMantenimientoCarritoCompras.EiminaArticuloDelCarrito,  Detalle: " + ex.getMessage().toString() +"]");
			Cls_Log.insertaLog(log);
		}
		finally 
		{
	...
	public boolean EnviaPedidoAMayorista(InfoCliente infoCliente, String pedido) {
			...
			// 	TimeUnit.SECONDS.sleep(1);
			 ...
			 //	{
			 		...
			 		while ((linea = rd.readLine()) != null) {
			 			...
			 			if(resultado.toString().equalsIgnoreCase("0"))
					    {
		...
		//	}
			log = new Log(1, infoCliente.getCve_cdo(), infoCliente.getCve_cliente(), Integer.parseInt(pedido),
					"Resultado a URL_WS_PEDIDO_WWW [" + JSON_SOURCE + "]");
			
			if(JSON_SOURCE.equalsIgnoreCase("0"))
		    {
		    	ejecucionCorrecta  = true;
		    }
	...
	public boolean EnviaPedido72Hrs(List<Querys> listaQuerys, InfoCliente infoCliente, String pedido) {
		...
		//	}
			log = new Log(1, infoCliente.getCve_cdo(), infoCliente.getCve_cliente(), Integer.parseInt(pedido),
					"Resultado a URL_WS_PEDIDO_WWW_72hrs [" + JSON_SOURCE + "]");
			...
			if(JSON_SOURCE.trim().equalsIgnoreCase(""))
		    {
		    	ejecucionCorrecta  = true;
		    }
			...
			float importe, String GeneraTraspaso, String cdoTraspaso)
	{
		...
		{
			System.out.println("[NuevoportalRamaCDO.- Error: Modificar Cantidades pedidas,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.toString() +"]");
			Log log=new Log(1,infoCliente.getCve_cdo(),infoCliente.getCve_cliente() ,0, "[Error: Modificar Cantidades pedidas,  Clase: GestorMantenimientoCarritoCompras,  Detalle: " + ex.getMessage().toString() +"]");
			Cls_Log.insertaLog(log);
		}
		finally 
		{
