...
//import com.cordina.PortalCordinaOEP.Util.clsLog;
//import com.cordina.PortalCordinaOEP.conexionBd.connectionManager;
//import com.cordina.PortalCordinaOEP.conexionBd.connectionManagerV2;
import datos.DelegacionDto;
import datos.EstadosDto;
import datos.clientesOEPDto;
import datos.clsLog;
import datos.productosOEPDto;
import util.connectionManagerV2;
...
public class ServletDondeComprar extends HttpServlet {
	...
	public void main(HttpServletRequest request, HttpServletResponse response)
	{
		try
		{
			...
			List<productosOEPDto> lisArticulos = new ArrayList<productosOEPDto>();
			List<DelegacionDto> listDelegaciones= new ArrayList<DelegacionDto>();
			
			List<clientesOEPDto> listClientes= new ArrayList<clientesOEPDto>();
			
		 	// System.out.println("ServletDondeComprar 2. INICIO");
			//System.out.println(request.getParameter("operacion"));
			try
			{
			...
			catch(Exception ex)
			{
				...
				//System.out.println("Portal OEP - ERROR1: " + sError);
			}
		   ...
		   //  System.out.println("ServletDondeComprar: INICIO OPERACION= " + operacion);
			...
			switch(operacion)
			{
				case 1:
					...
					lisArticulos = ObtieneProductosOEP();
					listClientes = ObtieneClientesOEP(0,pais);
				...
				case 2:			
					...
					{
						String sError= ex.getMessage().toString();
						System.out.println("Portal OEP - ERROR2: " + sError);
					}

					lisArticulos = ObtieneProductosOEP();
					if(km != null) {
						listClientes = ObtieneClientesOEP(producto,pais);
					}
				...
				case 4:
					...
					if(listaCliente != null && listaKM != null) {
						listClientes = ObtieneClientesOEPEspecificos(producto, clientesOrdenados, kmOrden,pais);
						lisArticulos = ObtieneProductosOEP();						
					}					 
					request.setAttribute("latitud2", "");
					request.setAttribute("longitud2", "");
			...
			default:
				...
				{
					String sError= ex.getMessage().toString();
					System.out.println("Portal OEP - ERROR3: " + sError);
				}
	...
	public List<clientesOEPDto> ObtieneClientesOEP(int articulo, int pais)
	{
		List<clientesOEPDto> listClientes= new ArrayList<clientesOEPDto>();
		...
		try
		{
			String Qry = "	SELECT 	DISTINCT A.cve_cliente_oep, "+
									"A.nombre, "+ 
									"A.razon_social,  "+
						...
						" FROM 		OEP.c_clientes_oep  AS A "+
									"JOIN OEP.c_cliente_producto_oep AS B ON A.cve_cliente_oep = B.cve_cliente_oep " +
									"LEFT JOIN KWX.c_delegaciones AS C ON A.cve_delegacion = C.cve_delegacion " +
									" LEFT JOIN KWX.c_sepomex AS s ON s.id_cp = A.cp ";
			
				if(articulo > 0)
				{
					Qry += " WHERE  B.cve_producto_oep = '" +  articulo + "' AND A.id_pais="+pais ;													
					Qry +=" ORDER BY  A.cve_cliente_oep ASC";
				}				
				else if(articulo == 0)
				{
					Qry += " WHERE A.id_pais='"+pais+"' ORDER BY  A.cve_cliente_oep ASC";
				}
			...
			{	
	    		while (rs.next())
	    		{
		    		try
		    		{
		    			...
		    			clientesOEPDto cte= new clientesOEPDto();
		    			cte.setCve_cliente(rs.getInt("cve_cliente_oep"));
		    			...
		    			cte.setCve_producto_oep(rs.getInt("cve_producto_oep"));
		...
		catch(Exception ex)
		{
			...
			if(connBD != null) {connBD.close();}
				...
				System.out.println("Error al Cerrar la conexion de ObtieneClientesOEP: " + ex.toString());
			}
		}
	...
	public List<clientesOEPDto> ObtieneClientesOEPEspecificos(int articulo, String listaClientes, String listaKM,int pais)
	{
		List<clientesOEPDto> listClientes= new ArrayList<clientesOEPDto>();
		Connection connBD = null;
		PreparedStatement pstmt = null;
		...
		{ 
			...
			String Qry = "	SELECT 	DISTINCT A.cve_cliente_oep, "+
					...
					{
						Qry += " WHERE  (B.cve_cliente_oep in ("+clientes+") and B.cve_producto_oep = '"+articulo+"') ";
						
					}				
					else if(articulo == 0 && !clientes.equals(""))
					{
						Qry += " WHERE A.cve_cliente_oep in ("+clientes+") "; 						
					}
					else if(articulo > 0 && clientes.equals("")) 
					{
						Qry += " WHERE  B.cve_producto_oep = '"+articulo+"'";
					}
					//System.out.println(Qry);
			connBD = connectionManagerV2.getConnection();
			pstmt = connBD.prepareStatement(Qry);
				...
				{	
		    		...
		    		{	
			    			...
			    			for(int i = 0; i<kmClieneSep.length;i++) {
			    				...
			    				String kmSe = cliKm[1];		    	
			    				cli = cli.replace(" ", "");
			    				if(cli.equalsIgnoreCase(String.valueOf(rs.getInt("cve_cliente_oep")))) {
			    					cte.setKm(kmSe);
			    					cte.setCve_cliente(rs.getInt("cve_cliente_oep"));
					    			cte.setNombre(rs.getString("nombre"));
					    			cte.setRazonSocial(rs.getString("razon_social"));
    		...
    		catch(Exception ex)
    		{
    			...
    			System.out.println("Error en ObtieneClientesOEPEspecificos en el Query: " + sError);
    			//CierraConexion();
    		}
		}
		catch(Exception ex)
		{
			String sError= ex.getMessage().toString();
			System.out.println("ObtieneClientesOEPEspecificos.sError: "+ex.toString()); 
			System.out.println("ObtieneClientesOEPEspecificos.sError: "+sError);
		}finally {
			try {
				if(pstmt != null) {pstmt.close();}
				if(connBD != null) { connBD.close();}
			}catch(Exception e) {
				System.out.println("Error ObtieneClientesOEPEspecificos al cerrar la conexion: " + e.toString());
			}
		...
		try
		{
			 Qry= "SELECT  DISTINCT A.id_cve_edo, "+
						...
						"FROM 	comercio_electronico.c_estados AS A "+
								"JOIN OEP.c_clientes_oep AS B ON A.id_cve_edo = B.id_cve_edo  " +
								"ORDER BY A.nombre;";
	...
	public List<productosOEPDto> ObtieneProductosOEP()
	{
		List<productosOEPDto> lisArticulos = new ArrayList<productosOEPDto>();
		Connection connBD = null;
		PreparedStatement pstmt = null;
		...
		try
		{
			String Qry= " SELECT cve_producto_oep, descripcion FROM OEP.c_producto_oep;";
			...
			if(rs != null)
			{
	    		while (rs.next())
	    		{
		    		try
		    		{
		    			productosOEPDto producto= new productosOEPDto();
		    			producto.setCve_producto_oep(rs.getInt("cve_producto_oep"));
		    		...
		    		{
		    			String sError= ex.getMessage().toString();
		    			System.out.println("Error en ObtieneProductosOEP Al consultar: " + sError);
		    			//CierraConexion();
		    		}
	    		}
			}
		...
		{
			String sError = ex.getMessage().toString();
			System.out.println("Error en ObtieneProductosOEP: En el QUERY: " + sError);
			//CierraConexion();
		}finally {
			try {
				if(pstmt != null) {pstmt.close();}
	...
	public ResultSet CrearConexionBaseDatos(String sql)
	{
		...
		try
			...
			}catch(Exception e)
			{
				CierraConexion();
				String sError= e.getMessage().toString();
				System.out.println("Portal OEP - ERROR EJECU. QUERY: " +  sError);
				clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CrearConexionBaseDatos", "getLocalizedMessage: "+ e.getLocalizedMessage().replace("'", ""));
				clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CrearConexionBaseDatos", "getMessage: "+ e.getMessage().replace("'", ""));
				clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CrearConexionBaseDatos", "getCause: "+ e.getClass().getName().replace("'", ""));
				clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CrearConexionBaseDatos", "getClass().getName: "+ e.getLocalizedMessage().replace("'", ""));
			}
		return rs1;
	}
	...
	public void CierraConexion()
	{
		...
		{
			String sError= e.getMessage().toString();
			System.out.println("Portal OEP - ERROR4: " +  sError);
			clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CierraConexion", "getLocalizedMessage: "+ e.getLocalizedMessage().replace("'", ""));
			clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CierraConexion", "getMessage: "+ e.getMessage().replace("'", ""));
			clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CierraConexion", "getCause: "+ e.getClass().getName().replace("'", ""));
			clsLog.RegistraLog("Portal OEP - ServletDondeComprar.CierraConexion", "getClass().getName: "+ e.getLocalizedMessage().replace("'", ""));
		}
	}
