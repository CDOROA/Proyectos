...
function cancelarUno(eId)
{
	...
	var claseOrdenImporte = document.getElementsByClassName("importeCheques");
	var claseOrdenMotivo = document.getElementsByClassName("motivoCheques");
	var claseOrdenBanco = document.getElementsByClassName("BancoHidden");
	
	
	var  arrayClaseOrdenFinal= [];
	var  arrayClaseImporte= [];
	var  arrayClaseMotivos= [];
	var  arrayClaseBancos= [];
	...
	for (var i = 0; i < claseOrdenBanco.length; ++i)
	{
		if (typeof claseOrdenBanco[i].value !== "undefined") 
		{	
			arrayClaseBancos.push(claseOrdenBanco[i].value);
		}
	}
	...
	else if (arrayClaseBancos[eId] == '') 
	{
		alert('Selecciona el banco deposito del cheque.');
		return  false;
	}
	return  true;
}
...
function MotivoInputHiddenBanco(s,e)
{
	let select = document.getElementById("_nSelectbancoEmisor");
	let n= s.split("_");
	document.getElementById(n[0]+"_bancoDeposito").value = document.getElementById(s).value	
}
