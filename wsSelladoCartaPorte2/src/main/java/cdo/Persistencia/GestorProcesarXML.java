...
public class GestorProcesarXML {
	...
	public Respuesta GenerarCFDICartaPorte(List<Querys> listaQuerys, Respuesta respuesta, String cdo, String uname_br, String id_trayecto) 
	{
		...
		catch (Exception e) 
		{
			...
			System.out.println("Error al generar CFDI Carta Porte: "+e.getMessage().toString());
        }
	...
	private void ejecutarInsertsEnBD(String cdo,List<Querys> ListaQuerys) 
	{
		...
		try
		{
			//System.out.println("ejecutarInsertsEnBD...1");
	        ...
	        try
	        {
	        	...
	        	//System.out.println("ejecutarInsertsEnBD...2: " + ListaQuerys);
	        	querys = inicializacionQrys(querys, ListaQuerys, cdo, 2);
	        	//System.out.println("ejecutarInsertsEnBD...3");
	           ...
	           // System.out.println("ejecutarInsertsEnBD...4");
	        }
	        catch (Exception e) 
	        {
	        	System.out.println("Error al ejecutatar qury en BD: " + e.getMessage());
	        	rsp = false;
			}
	        finally
	        {
	...
	private void consultarEmisorReceptor(List<Querys> ListaQuerys) 
	{
		if (rsp)
		{
			try
			{
		        ...
		        {
		        	System.out.println("Error al consultar emisor en BD+"+e.getMessage());
		        	rsp = false;
				}
		        finally
		        {
	...
	public static String[] ObtieneQuerys(int noQuery, List<Querys> ListaTodosQuerys, String[] ListaQuerys,  String cdo, XML objXML)
	{
		...
		for(int item = 0; item < ListaTodosQuerys.size(); item++)
		{
			...
			{
				//  System.out.println("ObtieneQuerys getSub_indice_query "+ListaTodosQuerys.get(item).getSub_indice_query());
	...
	private static String sentenciaComprobanteUbicaciones(String qry, String cdo) 
	{
		...
		{
			// System.out.println(" objXML.getUbicaciones().get(i).getDistancia_recorrida() : " +  objXML.getUbicaciones().get(i).getDistancia_recorrida());
			valores = valores + "('"+objXML.getComprobante().getUname()+"', '"+
		                             objXML.getComprobante().getSerie()+"', '"+
					                 objXML.getComprobante().getFolio()+"', '"+
		                             objXML.getComprobante().getCarta_porte()+"','"+
	...
	{
		String valores = "";
		//System.out.println("bjXML.getOperador().size: " + objXML.getOperador().size());
	...
	private void llenarUbicaciones() 
	{
		if (rsp)
		{
			try
			{
				...
				//System.out.println("llenarUbicaciones: " + totalSecciones );
				for (int i = 0; i < totalSecciones; i++) 
				{
					...
					//System.out.println("IDOrigen seccion: "+ seccion );
					...
					//System.out.println("objUbicaciones: " + objUbicaciones);
	...
	private void llenarDatosComprobante() 
	{
			...
			try
			{
				...
				//System.out.println("getCodigoPDF: " + objRespuesta.getCodigoPDF());
	...
	private void llenarMercancias() 
	{
		...
		if (rsp)
		{
			...
			{
				System.out.println("Error al llenar llenarMercancias. ERROR: " + e);
	        ...
	        if (!funcion.equals("size"))
			{
				...
				try
				{
					if(nNode.getNodeType() == Node.ELEMENT_NODE) 
					{
						...
						// System.out.println("VALOR. NODO: "+nodo+". CAMPO: "+campo + ". seccion: " + funcion + ". value: " + valor);
					}
				}
				catch (Exception e) 
				{
