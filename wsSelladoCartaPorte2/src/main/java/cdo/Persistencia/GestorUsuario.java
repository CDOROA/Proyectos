...
public class GestorUsuario
{
    public static List<Querys> ConsultaTablaQuerysBD(final String cdo) {
        ...
        try {
        	...
        	System.out.println("eRROR AL CONSULTAR LA TABLA DE QUERYS:" + ex);
        }
        finally {
            try {
                connBD.close();
        ...
        switch (numQuery) {
            case 2: {
                qry = "SELECT"
        				...
        				+ "where "
        				+ "proc_web = '113' " + "  ORDER BY indice_query ASC, " + "sub_indice_query ASC;";
                break;
            }
        }
        return qry;
    ...
    static String[] InicializaQueryNumero1(final String[] ListaQuerys, final String cdo, int cliente) 
    {
        for (int i = 0; i < ListaQuerys.length; ++i) 
        {
            ListaQuerys[i] = ListaQuerys[i].replace("{UNAME}", cdo.toLowerCase());
            ListaQuerys[i] = ListaQuerys[i].replace("{UNAME_BR}", cdo.toLowerCase());
            ListaQuerys[i] = ListaQuerys[i].replace("{CDO}", cdo.toUpperCase());
            ListaQuerys[i] = ListaQuerys[i].replace("{CLIENTE}", String.valueOf(cliente));
        }
        return ListaQuerys;
    }
	...
	public static String[] inicializacionQrys(String[] querys, List<Querys> ListaQuerys, String cdo, int noQry, int cliente) 
	{
