...
public class UsuarioServlet extends HttpServlet {
	...
	private void validaCredencialesDeUsuario(HttpServletRequest request, HttpServletResponse response,HttpSession session) {
		...
		try {
			...
			else
			{
				...
				RedireccionarVista(request, response,"../AdministradorCFDI?accion=Inicio");
			}
		} catch (Exception e) {
	...
	private void consultaQuerys(HttpSession session) {
		...
		this.querys = null;
		this.querys = Cls_Querys.ConsultaTablaQuerysBD(connBD,(String) session.getAttribute("cdo"));
