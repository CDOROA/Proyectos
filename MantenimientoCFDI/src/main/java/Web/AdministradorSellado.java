...
import Persistencia.GestorEstatus;
import Persistencia.GestorNotas;

/**
 * Servlet implementation class AdministradorSellado
 */
public class AdministradorSellado extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdministradorSellado() {
        super();
   			 ...
   			 JSONObject listaCFDIJson = new JSONObject();	
   			  //System.out.println("accion:" + accion);
   					...
   					case "Inicio":
   						...
   						RedireccionarVista(request, response, "/AdministradorSellado.jsp");
   						break;
   					case "Cargar":
   						listaCFDIJson=consultaEstatus(request,response, listaQuerys, session);
   						EviarRespuestaJsonJS(request,response, "["+ listaCFDIJson.toString()+"]");
   						break;
   					case "CambiaEstatus":
   						listaCFDIJson=cambiaEstatus(request,response, listaQuerys, session);
   						EviarRespuestaJsonJS(request,response, "["+ listaCFDIJson.toString()+"]");
   						break;
   						}
   	...
   	private JSONObject cambiaEstatus(HttpServletRequest request, HttpServletResponse response, List<Querys> listaQuerys,
			HttpSession session) {
   		JSONObject JsonEstatus = new JSONObject();
   		String cdo =  (String) session.getAttribute("uname");
   		Usuario infoUsu = (Usuario) session.getAttribute("DatosUsr");
   		String st=request.getParameter("estatus");
   		String estatus =  GestorEstatus.cambiaEstatus(listaQuerys,cdo,infoUsu,st);
		
   		JsonEstatus.put("estatus",estatus);
   		return JsonEstatus;
	}

	private JSONObject consultaEstatus(HttpServletRequest request, HttpServletResponse response,
			List<Querys> listaQuerys, HttpSession session) {
   		JSONObject JsonEstatus = new JSONObject();
   		String cdo =  (String) session.getAttribute("uname");
   		Usuario infoUsu = (Usuario) session.getAttribute("DatosUsr");
   		String estatus =  GestorEstatus.consultaEstatus(listaQuerys,cdo,infoUsu);
